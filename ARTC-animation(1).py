...
def patch_streamlit_for_kids():
    ...
    def w_selectbox(label, options, *args, **kwargs):
        ...
        if fmt is None:
            ...
            kwargs['format_func'] = (lambda f: (lambda x: translate_display(f(x)) if isinstance(f(x), str) else f(x)))(fmt)
    ...
    try:
        ...
        st._orig_sb_metric = _BASE_SB_METRIC if _BASE_SB_METRIC else (st.sidebar.metric if hasattr(st.sidebar, 'metric') else None)
...
def show_background():
    ...
    with col1:
        ...
        with col_a:
            ...
            st.image("https://via.placeholder.com/150x100/FF6B6B/FFFFFF?text=Tumor+Cells",
                     caption="Undifferentiated Tumor Cells")

        with col_b:
            st.markdown("**2. ATRA Action**")
            st.image("https://via.placeholder.com/150x100/4ECDC4/FFFFFF?text=ATRA",
                     caption="ATRA Molecule")

        with col_c:
            st.markdown("**3. Differentiation Maturation**")
            st.image("https://via.placeholder.com/150x100/45B7D1/FFFFFF?text=Mature+Cells",
    ...
    with col2:
        ...
        timeline_data = {
            ...
            'Event': ['ATRA Discovery', 'APL Treatment Breakthrough', 'Solid Tumor Research', 'Liver Cancer Clinical Trials', 'Engineered Bacteria Development'],
...
def show_basic_experiments():
    ...
    if is_kids_mode():
        st.info("Let's get hands-on! These are the most common little steps in the lab, just like preparing ingredients before making a cake~")
    ...
    with col2:
        ...
        if current_step >= len(steps):
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge={
                    'axis': {'range': [0, 100], 'tickwidth': 2, 'tickcolor': '#1E90FF', 'tickfont': {'color': '#37474f'}},
                    ...
                    'threshold': {'line' : {'color': '#0d47a1', 'width': 6}, 'thickness': 0.8, 'value': dissolution_progress * 100},   
...
def show_engineering_bacteria():
    ...
    if is_kids_mode():
        st.info("Let's build super bacteria together! We'll put the best genes together like building blocks to make tiny factories that create amazing things! ✓")

    tab1, tab2, tab3 = st.tabs(["Downstream Plasmid Construction", "Upstream Plasmid Construction", "Gene Integration Validation"])
...
def show_crispr_cas9():
    ...
    if is_kids_mode():
        st.info("CRISPR is like a super-precise pair of scissors that helps us 'cut and paste' on DNA! It's like editing a recipe to make something even more delicious!")
...
def show_results_analysis():
    ...
    if is_kids_mode():
        st.info("Time to see our results! Let's check if our experiments worked using pictures and numbers that tell us the story of our amazing bacteria!")

    tab1, tab2, tab3, tab4 = st.tabs(["Gene Expression Validation", "Protein Function Analysis", "Metabolite Detection", "Therapeutic Effect Evaluation"])
    ...
    with tab1:
        ...
        with col2:
            ...
            metabolic_genes = ['Carbohydrate Metabolism', 'Lipid Metabolism', 'Amino Acid Metabolism', 'Vitamin Metabolism', 'Pigment Synthesis']
